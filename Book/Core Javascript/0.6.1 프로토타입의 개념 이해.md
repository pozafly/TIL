...
Array를 new 연산자와 함게 호출해서 instance를 생성하든, 그냥 배열 리터럴을 생성하든, 어쨋든 instance인 `[1, 2]` 가 만들어진다. 이 인스턴스의 `__proto__` Array.prototype 을 참조하는데, `__proto__` 가 생략 가능하도록 설계돼 있기 때문에 잇느턴스가 push, pop, forEach 등의 메서드를 마치 자신의 것처럼 호출할 수 있다.
