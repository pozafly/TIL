...
## 답
1. 어떤 함수에서 선언한 변수를 참조하는 내부함수에서 발생하는 현상. 외부 함수가 반환된 후에도 외부 함수의 변수 볌위 체인에 접근할 수 있다. (다른 대답은, 어떤 함수 A에 선언된 변수 a를 참조하는 내부함수 B를 외부로 전달할 경우 A의 실행컨텍스트가 종료된 이후에도 변수 a가 사라지지 않는 현상)
2. 
   - 클로저로 데이터 프라이버스 / private method를 모방, 일반적으로 모듈 패턴에 사용된다.
