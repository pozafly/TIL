...
```mermaid
flowchart LR
    ADMIN[Admin Devices<br/>Tailscale Client]
...
- 외부 인터넷 경로: ISP -> OPNsense -> Proxmox/LAN
- 내부 서비스들은 Proxmox 위 VM/LXC로 존재
- Admin 기기는 직접 LAN에 붙는 게 아니라 Tailscale 네트워크를 통해 접속
- Tailscale Subnet Router LXC가 LAN subnet, 예를 들면 192.168.x/24, 10.x.x/24 등을 광고
- 그래서 Admin 기기에서 내부 VM/LXC 서비스로 접근 가능

```mermaid
flowchart TB
    subgraph INTERNET["Internet / Outside"]
      NET[Internet Clients]
      ADMIN[Admin Device<br/>Tailscale: 100.x]
    end

    subgraph UPSTREAM["Upstream / Home Router Network<br/>192.168.68.0/22"]
      RTR[Home Router<br/>192.168.68.1]
      PVEWAN[Proxmox vmbr0<br/>192.168.68.10/22]
    end

    subgraph PVE["Proxmox Host pve1"]
      PVEHOST[Proxmox Host<br/>vmbr0: 192.168.68.10<br/>tailscale: 100.100.26.110]

      subgraph OPN["OPNsense VM"]
        OPNWAN[WAN NIC<br/>vmbr0 / 192.168.68.x]
        OPNLAN[LAN NIC<br/>vmbr1 / 192.168.10.1]
      end

      VMBR1[vmbr1<br/>Internal LAN bridge<br/>no Proxmox IP]

      TSMGMT[ts-mgmt LXC<br/>eth0: 192.168.10.10<br/>tailscale: 100.104.73.112<br/>advertises 192.168.10.0/24]

      OPENCLAW[OpenClaw LXC<br/>192.168.10.100<br/>gw 192.168.10.1]
      YUNA[Yuna LXC<br/>192.168.10.101<br/>gw 192.168.10.1]
      YUNATEST[Yuna-Test LXC<br/>192.168.10.102<br/>no default gw currently]
    end
    ...
    ADMIN -.Tailscale overlay.-> TSMGMT
```
